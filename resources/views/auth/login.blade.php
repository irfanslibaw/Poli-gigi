...
<head>
  ...
  <style>
        body {
            ...
            height: 100vh;
            background-image: linear-gradient(-45deg, #5C7793, #F8F9D7);
        }
  </style>
</head>
    ...
    <div class="w-9/12 p-12 bg-white sm:w-8/12 md:w-1/2 lg:w-5/12 rounded-xl">
        ...
        <div class="flex justify-center">
            ...
            @if (Route::has('password.request'))
                <p class="justify-between inline-block mt-4 text-sm text-gray-500">Lupa Password?<a href="{{ route('password.request') }}"> Reset Password</a></p><br>
            @endif
                <p class="justify-between inline-block mt-4 text-sm text-gray-800">Atau</p><br>
                <p class="justify-between inline-block mt-4 text-sm text-gray-500">Belum memiliki akun? <a href="{{ route('register') }}" class="cursor-pointer hover:text-blue-800">Register</a></p><br>
                </div>
        </form>
    </div>
  </div>
